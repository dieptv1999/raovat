...
import {SEO_DEFAULT_URL, SESSION, USER_ID} from "../../utils/constant";
...
export default function SingleProductPage({product}) {
    ...
    const fetchMarketMore = async () => {
        const create_dt = relatedNews?.length > 0 ? relatedNews[relatedNews.length - 1]?.create_dt : null;
        setRelatedLoading(true)
        const resp = await ApiFactory.getRequest("ProductApi").getListSellMarketMore({
        ...
        if (resp.success) {
            setRelatedNews(!!create_dt ? [...(relatedNews ?? []), ...resp.listSell] : resp.listSell)
        } else setRelatedNews([])
    }
                    ...
                    <div className="related-product w-full bg-white">
                        <div className="container-x mx-auto">
                            <div className="w-full py-[60px]">
                                ...
                                {relatedLoading ? <ListCardLoading/> : <div/>}
                                <div className="flex justify-center my-6">
                                    <button className="btn btn-warning" onClick={fetchMarketMore}>Xem thêm</button>
                                </div>
                            </div>
                        </div>
                    </div>
