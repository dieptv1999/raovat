...
export default function AddressesTab() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const resp = await ApiFactory.getRequest("UserApi").getListMyFollower({
      user_id: user.userId,
    })
    if (resp && resp.success) {
      setData(resp?.listFollower)
    }
  }, [user?.userId])

  const unfollow = useCallback(async (receive) => {
    if (!user.userId) return;
    ...
    if (!resp.success) {
      ...
      utils.showMessage('Thông báo', 'Bỏ theo dõi thất bại', 'error')
    }
  ...
  return (
    <>
      <div className="mb-2 text-lg md:text-xl font-semibold">Danh sách người bạn đang theo dõi</div>
      <div className="grid grid-cols-2 gap-[30px]">
        {data.map(user => (
            <div key={user.receive} className="w-full bg-primarygray p-5 border rounded shadow">
              <div className="flex justify-between items-center">
                <div className='h-[50px] md:h-[60px] aspect-square rounded-full overflow-hidden
                            relative'>
                  <Image
